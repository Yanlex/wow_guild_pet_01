...
export default function RegistrationForm() {
  ...
  console.log('Registration.jsx mount')
    ...
    <form onSubmit={handleSubmit(onSubmit)} className='regForm'>
      {
        status == 'Юзер успешно зарегистрирован' ? (<div style={{ color: 'black', padding: '10px', margin: '0 0 10px 0', backgroundColor: 'rgba(59, 219, 91, 0.6)', lineHeight: '20px', textAlign: 'center' }}><p>{status}</p></div>) : (<>
          ...
          <input type="email" autoComplete="off" {...register("email", { pattern: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i })} placeholder='Емейл' />
          <input type="password" autoComplete="off" {...register("password", {
            pattern: /^(?=.*[0-9])(?=.*[a-zA-Zа-яА-ЯёЁ])(?=.*[*.!@$%^&(){}[\]:;<>,.?/~_ +-=|\\]).{8,32}$/
          })} placeholder='Пароль' />
          <label for="password" style={{ marginLeft: '20px' }}><ul>
            <li style={/^.{8,}$/.test(passwordUseWatch) ? green : normalStyle}>Длина пароля не менее 8 символов</li>
            <li style={/\d/.test(passwordUseWatch) ? green : normalStyle}>Пароль должен содержать 1 цифру</li>
            <li style={/[\W_]/.test(passwordUseWatch) ? green : normalStyle}>Пароль должен содержать 1 знак</li>
            <li style={/[\p{Lu}]/u.test(passwordUseWatch) ? green : normalStyle}>Пароль должен содержать 1 заглавную букву</li>
            <li style={/[\p{Ll}]/u.test(passwordUseWatch) ? green : normalStyle}>Пароль должен содержать 1 букву в нижнем регистре</li>
          </ul></label>
