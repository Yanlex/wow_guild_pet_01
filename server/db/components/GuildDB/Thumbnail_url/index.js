...
db.serialize(async () => {
	...
	const row = await new Promise((resolve, reject) => {
		db.get(
			`SELECT COUNT(*) as cnt FROM sqlite_master WHERE name='members' and type='table' and sql LIKE '%thumbnail_url%'`,
			[],
			(err, row) => {
				if (err) {
					reject(err);
		...
		https
			.get(url, (res) => {
				...
				res.on('end', () => {
					...
					db.run(
						'UPDATE members SET thumbnail_url = ? WHERE character_name = ?',
						[thumbnail, name],
						(err) => {
							if (err) {
